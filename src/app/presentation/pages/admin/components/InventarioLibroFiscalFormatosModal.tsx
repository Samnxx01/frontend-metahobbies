...
export default function InventarioLibroFiscalFormatosModal({
  open,
  libro,
  onOpenChange,
  ...
  React.useEffect(() => {
    ...
    setDraft(blankRow());
    inventarioService
      .listarFormatosLibroFiscal(libro.iud)
      .then((data) => {
      ...
      .catch((err) => {
        if (mounted) {
          setRows([]);
          toast.error(err instanceof Error ? err.message : 'No se pudieron cargar los formatos.');
        }
      })
      .finally(() => {
  ...
  const guardar = async (): Promise<void> => {
    ...
    setSaving(true);
    try {
      const saved = await inventarioService.guardarFormatosLibroFiscal(libro.iud, rows);
      setRows(saved);
      toast.success('Formatos guardados.');
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'No se pudieron guardar los formatos.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100%-2rem)] max-h-[90dvh] max-w-3xl overflow-y-auto border-border bg-background text-foreground">
        <DialogHeader>
          <DialogTitle>Formatos de {libro?.codigo || 'libro fiscal'}</DialogTitle>
        </DialogHeader>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4} className="py-6 text-center text-sm text-muted-foreground">
                    No hay formatos parametrizados.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
