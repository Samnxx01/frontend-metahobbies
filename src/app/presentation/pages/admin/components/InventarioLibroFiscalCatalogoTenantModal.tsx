...
import inventarioService, { type InventarioLibroFiscalCatalogoHijo } from '@/app/services/inventarioService';
...
export type CatalogoTenantModo = 'tipos-libro' | 'periodicidades';

type Props = {
  open: boolean;
  modo: CatalogoTenantModo;
...
const titulos: Record<CatalogoTenantModo, string> = {
  'tipos-libro': 'Tipos de libro fiscal',
  periodicidades: 'Periodicidades de libro fiscal',
};

export default function InventarioLibroFiscalCatalogoTenantModal({
  open,
  modo,
  onOpenChange,
  onSaved,
  ...
  const cargar = React.useCallback(async () => {
    if (modo === 'tipos-libro') return inventarioService.listarTiposLibroFiscalCatalogo();
    return inventarioService.listarPeriodicidadesLibroFiscalCatalogo();
  }, [modo]);

  const guardarCatalogo = React.useCallback(async (data: InventarioLibroFiscalCatalogoHijo[]) => {
    if (modo === 'tipos-libro') return inventarioService.guardarTiposLibroFiscalCatalogo(data);
    return inventarioService.guardarPeriodicidadesLibroFiscalCatalogo(data);
  }, [modo]);
  ...
  React.useEffect(() => {
    ...
    cargar()
      ...
      .catch((err) => {
        if (mounted) {
          setRows([]);
          toast.error(err instanceof Error ? err.message : 'No se pudo cargar el catalogo.');
        }
      })
      .finally(() => {
  ...
  const guardar = async (): Promise<void> => {
    if (!rows.some((row) => row.activo)) {
      toast.error('Debe existir al menos un registro activo.');
      return;
    }
    setSaving(true);
    try {
      const saved = await guardarCatalogo(rows);
      setRows(saved);
      toast.success('Catalogo guardado.');
      onSaved?.();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'No se pudo guardar el catalogo.');
    } finally {
      setSaving(false);
    }
  };
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4} className="py-6 text-center text-sm text-muted-foreground">
                    No hay registros parametrizados.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
