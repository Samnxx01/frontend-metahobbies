...
export default function DocumentosCorporativos() {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await apiFetch(
          'https://server-mabs-xo9s.onrender.com/api/config/parametrizacion/listar/documentos/coporativa',
          { method: 'GET' }
        );
        setData(res);
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiFetch('/api/config/parametrizacion/guardar/certificados/coporativa', {
        method: 'POST',
        body: formData,
        headers: { }, // No poner Content-Type, el navegador lo gestiona
      });
      setFile(null);
      // Opcional: recargar lista de documentos
    } catch (err: any) {
        ...
        {loading ? (
          ...
          <div className="mb-4">
            {Array.isArray(data?.documentos) && data.documentos.length > 0 ? (
                  ...
                  <tbody>
                    {data.documentos.map((doc: any) => (
                      <tr key={doc._id} className="border-b">
                        ...
                        <td className="px-2 py-1">{doc.mimetype}</td>
                        <td className="px-2 py-1">{doc.usuario}</td>
              ...
              <div className="text-muted-foreground text-sm">No hay documentos registrados.</div>
            )}
          </div>
        )}
        <form onSubmit={handleSubmit} className="mb-4 flex flex-col gap-2">
          <input type="file" accept=".png,.jpg,.jpeg" onChange={handleFileChange} />
          <Button type="submit" disabled={submitting || !file}>{submitting ? 'Subiendo...' : 'Subir Documento'}</Button>
        </form>
      </CardContent>
    </Card>
  );
